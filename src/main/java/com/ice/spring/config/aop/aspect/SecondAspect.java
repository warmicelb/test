...
//    }
    ...
    public void doSecondBefore(JoinPoint joinPoint){
        System.out.println(joinPoint.getSignature().getName());
        System.out.println("second这里进行方法执行前的日志打印");
    }

    @After("execution(public int com.ice.spring.config.aop.config.MyCalculator.calculate(int,int))")
    public void doSecondAfter(){
        System.out.println("second这里进行方法执行后的日志打印");
    }
    ...
    public void doSecondAfterReturning(Object result){
        System.out.println("second这里进行方法执行返回的日志打印,result:"+result);
    }
    ...
    public void doSecondAfterThrowing(Exception exception){
        System.out.println("second这里进行方法执行异常的日志打印"+exception.getMessage());
    }

    @Around("execution(public int com.ice.spring.config.aop.config.MyCalculator.calculate(int,int)))")
    public Object doSecondAround(ProceedingJoinPoint proceedingJoinPoint){
        System.out.println("second环绕通知，执行方法之前");
        Object result = null;
        try {
            result = proceedingJoinPoint.proceed();
        } catch (Throwable throwable) {
            System.out.println("second环绕通知，捕获异常后");
            throwable.printStackTrace();
        }
        System.out.println("second环绕通知，执行方法之后");
        return result;
    }
